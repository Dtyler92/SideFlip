...
export default async function handler(req, res) {
  ...
  let analyticsReconciliationPending = false
  try {
    ...
    // Queue external analytics erasure and purge undelivered events while the
    // deletion lease is still retryable, and always before Auth deletion.
    const { error: analyticsDeletionError } = await supabase.rpc('queue_analytics_deletion', { p_user_id: user.id })
    if (analyticsDeletionError) {
      // Product-account deletion must not be blocked by analytics. The durable
      // tombstone lets the privileged worker repair a missing erasure request.
      analyticsReconciliationPending = true
      console.error('Analytics deletion queue failed:', analyticsDeletionError.message)
    }
    const analyticsState = analyticsDeletionError ? 'retry_pending' : 'queued'
    const { data: authClaim, error: authClaimError } = await supabase.from('account_deletion_tombstones').update({ status: 'auth_deleting', provider_cleanup_status: { storage: 'complete', auth: 'deleting', analytics: analyticsState } }).eq('user_id', user.id).eq('deletion_lease_id', leaseId).eq('status', 'processing').gt('deletion_lease_expires_at', new Date().toISOString()).select('user_id').maybeSingle()
    ...
    const { error: completionError } = await supabase.from('account_deletion_tombstones').update({ status: 'completed', completed_at: new Date().toISOString(), provider_cleanup_status: { storage: 'complete', auth: 'complete', analytics: analyticsState } }).eq('user_id', user.id).eq('deletion_lease_id', leaseId)
    if (completionError) throw completionError
    return res.status(200).json({ deleted: true, reconciliationPending: analyticsReconciliationPending })
  } catch (error) {
