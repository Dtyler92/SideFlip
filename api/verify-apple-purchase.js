...
export function createVerifyApplePurchaseHandler({
  client,
  verifySignedData = verifyAppleSignedData,
  apiClientFactory = createAppleServerApiClient,
  getCurrentSubscription = currentAppleSubscription,
  ...
  return async function handler(req, res) {
    ...
    const { data: { user }, error: authError } = await runtimeClient.auth.getUser(token)
    if (authError || !user) return res.status(401).json({ error: 'Please sign in again.' })
    try {
      const { verifier: appleVerifier, decoded: transaction, environmentName: verifiedEnvironment } = await verifySignedData(req.body.signedTransaction)
      ...
      const current = await getCurrentSubscription({ apiClient: apiClientFactory(verifiedEnvironment), verifier: appleVerifier, transactionId: transaction.transactionId, expectedOriginalTransactionId: transaction.originalTransactionId })
      ...
      const providerSignedAt = new Date(current.providerSignedDate).toISOString()
      const { data: applied, error: writeError } = await runtimeClient.rpc('apply_apple_entitlement_event', {
        p_user_id: user.id,
...
export default createVerifyApplePurchaseHandler()
