...
-- Store a bounded, user-confirmed transmission type on Projects. Adding a
-- nullable text column is compatible with existing Project rows and clients.
alter table public.projects add column if not exists transmission text;
do $$ begin
  if not exists (
    select 1 from pg_constraint
    where conrelid='public.projects'::regclass and conname='projects_transmission_length'
  ) then
    alter table public.projects add constraint projects_transmission_length check (length(coalesce(transmission,'')) <= 200) not valid;
  end if;
end $$;

-- Replace only the V3 transfer implementation so newly transferred items
-- receive the confirmed Project transmission without changing V1/V2 RPCs.
do $$ begin
