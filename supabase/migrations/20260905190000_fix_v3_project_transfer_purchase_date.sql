...
-- Production projects never had purchase_date. Keep acquired_on unknown
-- rather than inventing a purchase date from created_at.
do $$ begin
...
begin
 ...
 select p.id,p.title,p.category,p.notes,p.photo,p.before_photo,p.after_photo,p.vehicle_year,p.vehicle_make,p.vehicle_model,p.model_number,p.serial_number,p.engine_model,p.engine_serial,p.vin,p.hull_number,p.purchase_price,p.status,p.created_at
