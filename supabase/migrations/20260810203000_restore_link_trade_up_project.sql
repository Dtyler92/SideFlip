-- Restore the existing-project Trade-Up Goal assignment RPC after detected production schema drift.
-- This is additive: do not rewrite or replay the previously recorded 20260803195329 migration.
...
begin
  ...
  select id, goal_id, goal_funding_amount
    into v_existing_id, v_existing_goal_id, v_existing_goal_funding
  from public.projects
  where user_id = v_user_id and trade_up_mutation_id = p_mutation_id;

  if v_existing_id is not null then
    if v_existing_id = p_project_id
       and v_existing_goal_id = p_goal_id
       and v_existing_goal_funding = coalesce(p_goal_funding, 0) then
  ...
  -- Re-check idempotency after obtaining both accounting locks.
  select id, goal_id, goal_funding_amount
    into v_existing_id, v_existing_goal_id, v_existing_goal_funding
  from public.projects
  where user_id = v_user_id and trade_up_mutation_id = p_mutation_id;
...
-- Reapply the least-privilege ACLs from the recorded Free/Pro goal-limit migration.
-- Production drift currently leaves create_trade_up_goal executable by PUBLIC,
-- although its auth.uid() guard still rejects unauthenticated calls.
...
comment on function public.link_trade_up_project(uuid, uuid, numeric, text)
  is 'Atomically links an authenticated user-owned active project to a user-owned active Trade-Up Goal.';
