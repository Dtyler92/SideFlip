...
export async function processLeasedJob({ lease, config, domains, provider, db }) {
  ...
  try {
    ...
    const discovered = await provider.discover({ asset, domains, maxSearches: config.maxSearches, maxFetches: config.maxFetches })
    if (!discovered || !Number.isSafeInteger(discovered.usage?.costNanoDollars) || discovered.usage.costNanoDollars < 0 ||
    ...
    const normalized = await provider.normalize({ evidence: registryToJson(registry) })
    if (!normalized || !Number.isSafeInteger(normalized.usage?.costNanoDollars) || normalized.usage.costNanoDollars < 0) {
    ...
    const costCents = Math.ceil((discovered.usage.costNanoDollars + normalized.usage.costNanoDollars) / 10_000_000)
    ...
    await db.settle({ jobId: lease.id, leaseToken: lease.lease_token, costCents, evidence: registryToJson(registry), candidates, unresolved })
    return { jobId: lease.id, candidateCount: candidates.length }
  } catch (error) {
    const code = safeErrorCode(error)
    await db.fail({ jobId: lease.id, leaseToken: lease.lease_token, code, detail: code })
    throw error
  }
}
