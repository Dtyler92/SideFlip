...
export default function MyStuffDetail() {
  ...
  const [integrityRollout, setIntegrityRollout] = useState({ featureEnabled:false, legacyRetired:false, installed:false })
  ...
  async function saveReading(event) {
    ...
    const mutationId = mutationIdForPayload(readingAttempt.current, integrityRollout.featureEnabled ? { itemId:id, readingType:reading.type, value } : wire)
    await run(() => integrityRollout.featureEnabled
      ? integrityClient.current.recordCurrentReading(id, reading.type, value, mutationId)
      : recordMyStuffReadingV2(wire, mutationId), async () => {
  ...
  async function removeItem() {
    if (integrityRollout.featureEnabled) {
      if (deleteText !== 'DELETE' || !window.confirm('Final confirmation: permanently delete this item, its history, and private media?')) return
      const descriptor = { itemId:id, action:'delete-v4' }
      const mutationId = mutationIdForPayload(deletionAttempt.current, descriptor)
      await run(async () => {
        const request = await integrityClient.current.requestItemDeletion(id, mutationId)
        resetMutationAttemptState(deletionAttempt.current)
        setDeletionRequestId(request.id)
        setDeletionNotice('Deletion is queued. SideFlip will confirm when cleanup is complete.')
      })
      return
    }
  ...
  async function printIntegrityReport() {
    try {
      ...
      const disclaimer = reportWindow.document.createElement('p'); disclaimer.textContent = report.owner_report_disclaimer || 'Owner-reported SideFlip records.'
      const content = reportWindow.document.createElement('pre'); content.textContent = JSON.stringify(report, null, 2); content.style.whiteSpace = 'pre-wrap'
      reportWindow.document.body.append(heading, disclaimer, content)
      reportWindow.setTimeout(() => reportWindow.print(), 100)
    } catch (next) { setError(next.message || 'Could not create the maintenance report.') }
  }
  ...
  return <main className="mystuff-shell" aria-labelledby="item-heading">
    ...
    {detailView === 'maintenance' && <MyStuffMaintenancePanel item={item} onChanged={load} integrityRollout={integrityRollout}/>}
    {detailView === 'history' && <MyStuffMaintenancePanel item={item} onChanged={load} mode="history" integrityRollout={integrityRollout}/>}
