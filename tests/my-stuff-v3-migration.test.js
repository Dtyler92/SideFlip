...
const projectYearFixUrl = new URL('../supabase/migrations/20260906010000_fix_project_transfer_model_year.sql', import.meta.url)
...
function projectYearFix() { return readFileSync(projectYearFixUrl, 'utf8') }
...
test('Project transfer model-year hotfix safely converts four-digit text years', () => {
  const sql = projectYearFix()
  assert.match(sql, /to_regprocedure\('public\.transfer_project_to_my_stuff_v3\(uuid,jsonb,text\)'\)/i)
  assert.match(sql, /create or replace function public\.transfer_project_to_my_stuff_v3\(/i)
  assert.match(sql, /nullif\(trim\(v_project\.vehicle_year::text\),''\)\s*~\s*'\^\[0-9\]\{4\}\$'/i)
  assert.match(sql, /trim\(v_project\.vehicle_year::text\)::integer between 1800 and 2200/i)
  assert.match(sql, /then trim\(v_project\.vehicle_year::text\)::integer else null end else null end/i)
  assert.doesNotMatch(sql, /\bp\.purchase_date\b|\bv_project\.purchase_date\b|transfer_project_to_my_stuff_v2\s*\(/i)
  assert.match(sql, /revoke all on function public\.transfer_project_to_my_stuff_v3\(uuid,jsonb,text\) from public,anon/i)
  assert.match(sql, /grant execute on function public\.transfer_project_to_my_stuff_v3\(uuid,jsonb,text\) to authenticated/i)
})
