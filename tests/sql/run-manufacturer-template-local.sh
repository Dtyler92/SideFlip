...
python3 "$ROOT/tests/sql/manufacturer-template-roundtrip-local.py" "$DB"
node --experimental-default-type=module "$ROOT/tests/sql/manufacturer-template-ingestion-local.mjs" "$DB"
printf 'PostgreSQL reusable manufacturer templates passed (local only)\n'
