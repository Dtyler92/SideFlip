...
env -i PATH="$PATH" HOME=/root node --experimental-default-type=module "$ROOT/tests/sql/document-job-local.mjs" "$DB"
printf 'PostgreSQL document job storage boundary passed (local only; in-memory job protocol)\n'
