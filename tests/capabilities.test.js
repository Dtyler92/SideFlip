...
import { can, canCreateGoal, getPlan } from '../src/capabilities.js'
...
test('Free users can create one active goal while Pro users can create more', () => {
  const activeGoal = [{ id: 'goal_1', status: 'active' }]
  const completedGoal = [{ id: 'goal_1', status: 'completed' }]

  assert.equal(canCreateGoal({}, null, []), true)
  assert.equal(canCreateGoal({}, null, activeGoal), false)
  assert.equal(canCreateGoal({}, null, completedGoal), true)
  assert.equal(canCreateGoal(LEGACY_STRIPE_PRO, null, activeGoal), true)
  assert.equal(canCreateGoal({}, ACTIVE_APPLE_PRO, activeGoal, TEST_NOW), true)
})
